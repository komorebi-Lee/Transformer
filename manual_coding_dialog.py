...
class ManualCodingDialog(QDialog):
        ...
        try:
            ...
            def on_ok():
                ...
                for item in first_level_items:
                    ...
                    if grandparent and grandparent.data(0, Qt.UserRole) and grandparent.data(0, Qt.UserRole).get("level") == 3:
    ...
    def highlight_text_by_code_id(self, code_id: str):
        ...
        try:
            ...
            if content_to_highlight:
                ...
                if clean_content:
                    ...
                    while content_search_count < max_searches:
                        content_cursor = self.text_document.find(clean_content, content_cursor, QTextDocument.FindCaseSensitively)
            ...
            if found and first_match_cursor:
                ...
                self.statusBar().showMessage(f"已高亮编码 {code_id} 及其对应内容，定位到第一个匹配项") if hasattr(self, 'statusBar') else None
            else:
        ...
        try:
            ...
            if code_id:
                ...
                while code_search_count < max_searches:
                    ...
                    code_highlight_format.setBackground(QColor(255, 255, 0))  # 黄色背景
                    code_highlight_format.setForeground(QColor(255, 0, 0))  # 红色文字
            ...
            if found and first_match_cursor:
                ...
                self.text_display.setTextCursor(first_match_cursor)
                self.text_display.ensureCursorVisible()  # 确保光标位置可见
                self.statusBar().showMessage(f"已高亮内容: {clean_content[:50]}...，并定位到第一个匹配项") if hasattr(self, 'statusBar') else None
            else:
        ...
        try:
            ...
            for third_cat, second_cats in self.current_codes.items():
                ...
                for second_cat, first_contents in second_cats.items():
                    ...
                    for content in first_contents:
                        ...
                        first_item.setText(4, str(len(first_sentence_sources) if first_sentence_sources else sentence_count))  # 句子来源数
                    ...
                    second_item.setText(4, str(len(second_sentence_sources)) if second_sentence_sources else "0")  # 句子来源数
                ...
                third_item.setText(4, str(len(third_sentence_sources)) if third_sentence_sources else "0")  # 句子来源数
            ...
            for first_code in self.unclassified_first_codes:
                ...
                if isinstance(first_code, dict):
                    ...
                    numbered_content = first_code.get('numbered_content', f"{code_id}: {content_text}" if code_id else content_text)
    ...
    def add_first_level_direct(self):
        ...
        try:
            ...
            self.statusBar().showMessage(f"已添加一阶编码: {code_id} - {clean_content}") if hasattr(self, 'statusBar') else None
        ...
        try:
            ...
            if level == 1:
                ...
                if parent:
                    ...
                    self.statusBar().showMessage(f"通过拖放添加一阶编码: {clean_content}，并更新目标编码文本来源") if hasattr(self, 'statusBar') else None
                    ...
                    self.statusBar().showMessage(f"通过拖放添加未分类一阶编码: {clean_content}") if hasattr(self, 'statusBar') else None
                ...
                if level == 2:
                    ...
                    self.statusBar().showMessage(f"通过拖放添加一阶编码到二阶'{second_name}': {clean_content}") if hasattr(self, 'statusBar') else None
