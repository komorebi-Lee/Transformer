...
class MainWindow(QMainWindow):
    ...
    def generate_codes_auto(self):
        ...
        try:
            ...
            # 使用最新的自动编码结果，增量更新二阶三阶库
            try:
                if self.structured_codes:
                    code_library = CodeLibrary("二阶三阶库.txt")
                    changed = code_library.update_with_structured_codes(self.structured_codes)
                    if changed:
                        code_library.save()
                        logger.info("已根据自动编码结果更新二阶三阶库")
            except Exception as lib_err:
                # 不影响自动编码主流程，仅记录日志
                logger.error(f"根据自动编码更新二阶三阶库失败: {lib_err}")
    ...
    def on_tree_item_double_clicked(self, item, column):
        ...
        code_id = item_data.get("code_id", "")
        content = item_data.get("content", "")
        sentence_details = item.data(0, Qt.UserRole).get("sentence_details", [])

        if not code_id and sentence_details:
            first_detail = sentence_details[0]
            code_id = first_detail.get("code_id", "") or first_detail.get("sentence_id", "")

        if not code_id and sentence_ids:
            first_id = sentence_ids[0] if isinstance(sentence_ids, list) else sentence_ids
            code_id = str(first_id).strip('[]')
